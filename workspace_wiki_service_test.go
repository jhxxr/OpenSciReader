...
- Transformers rely on attention layers.`) + "\n"
	...
	if job.Status != WorkspaceWikiScanJobCompleted {
		t.Fatalf("job.Status = %q, want %q (message=%q error=%q)", job.Status, WorkspaceWikiScanJobCompleted, job.Message, job.Error)
	}
	if llm.callCount != 1 {
		t.Fatalf("llm.callCount = %d, want 1", llm.callCount)
	}
	...
	if _, err := os.Stat(overviewPage.MarkdownPath); err != nil {
		t.Fatalf("Stat(overviewPage.MarkdownPath) error = %v", err)
	}
	if _, err := os.Stat(documentPage.MarkdownPath); err != nil {
		t.Fatalf("Stat(documentPage.MarkdownPath) error = %v", err)
	}
}
